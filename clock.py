...
parser.add_argument('--serial_port', type=str, default='/dev/ttyS0', help='Serial port to use (default: /dev/ttyS0)')
...
def send_time(last_sent_time: Optional[str] = None):
    # Get current time
    now = datetime.now()
    time_str = now.strftime("%H%M")  # Format time as a four-digit string
